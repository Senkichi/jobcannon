...
_jd_full.py): both the INSERT and UPDATE branches wrap their write in
`with raw.transaction():` rather than a bare raw.commit() call, so this also
works when `conn` is already inside an ambient `with conn.transaction():`
block (tests/host/conftest.py's db_conn fixture) — psycopg3 forbids explicit
commit() there; nesting degrades to a savepoint automatically.
"""
...
def upsert_job(
    ...
    *,
    ...
    if not isinstance(parsed, (ParsedJob, UnresolvedParsedJob)):
        raise TypeError(f"upsert_job requires ParsedJob | UnresolvedParsedJob, got {type(parsed)!r}")
    ...
    if existing is None:
        ...
        with raw.transaction():
            raw.execute(
                ...
                """,
                (
                    parsed.dedup_key, company_id, parsed.title, parsed.company,
                    parsed.location or None, Jsonb(parsed.locations_raw),
                    Jsonb([_loc_dict(l) for l in parsed.locations_structured]) if parsed.locations_structured else None,
                    parsed.workplace_type if parsed.workplace_type != "UNSPECIFIED" else None,
                    parsed.primary_country_code,
                    Jsonb(list(parsed.sources)), Jsonb(list(parsed.source_urls)),
                    parsed.source_id, Jsonb(sightings), parsed.description,
                    parsed.salary_min, parsed.salary_max, parsed.salary_currency,
                    parsed.salary_period, Jsonb(list(parsed.salary_observations)),
                    pd_date, pd_precision, None, ats_platform, None, None,
    ...
    for i, src in enumerate(parsed.sources):
        ...
        for entry in sightings:
            ...
            sightings.append({"source": src, "source_url": url, "first_seen": now_iso, "last_seen": now_iso})
    ...
    if salary_min is None and salary_max is None and (
        ...
        canonical_changed = True
    salary_observations = list(existing["salary_observations"] or []) + list(parsed.salary_observations)
    ...
    if locations_structured is None and parsed.locations_structured:
        locations_structured = [_loc_dict(l) for l in parsed.locations_structured]
        canonical_changed = True
    ...
    with raw.transaction():
        raw.execute(
            ...
            """,
            (
                Jsonb(sources), Jsonb(source_urls), Jsonb(sightings),
                new_pd, new_pd_precision, description,
                salary_min, salary_max, salary_currency, salary_period,
                Jsonb(salary_observations),
                location, Jsonb(locations_raw),
                Jsonb(locations_structured) if isinstance(locations_structured, list) else locations_structured,
                workplace_type, primary_country_code, ats_platform_col, source_id_col,
                matched_dedup_key,
            ),
        )
