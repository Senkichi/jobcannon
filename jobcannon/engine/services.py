...
#362 item 1, and #373 item 1 in accessor form — ``optional(svc,
"field_name")`` returning the attribute or None so the caller writes
``if optional(svc, "X") is None: skip`` — which is the same name-keyed
getattr in getter form: it erases static checking of the field name,
widens the returned attribute to Any, and is longer than the direct
``svc.X is None`` it replaces. Same rejection.
