...
def cascade_delete_user(conn, user_id: str) -> None:
    ...
    if pseudonym is None:
        return
    try:
        task_app.app.configure_task(
            PURGE_POSTHOG_PERSON_TASK,
            allow_unknown=True,
            queue=_PURGE_POSTHOG_PERSON_QUEUE,
        ).defer(distinct_id=pseudonym)
    except Exception:
        # Deferring itself is a local DB write (an INSERT into
        # procrastinate_jobs through the same connection pool), not a
        # PostHog call -- a failure here is a genuine local problem, but
        # the user's own deletion has already committed above, so this
        # logs rather than raising and turning a successful deletion into
        # a 500 (which would make Clerk retry the whole webhook delivery).
        logger.exception(
...
def _lookup_clerk_user(clerk_users, user_id: str) -> str:
    """Returns "present" | "not_found" | "error". Only a DEFINITIVE 404
    from Clerk's Backend API is "not_found" -- `models.ClerkErrors` carries
    the real HTTP status (verified against clerk_backend_api's SDK: `Users.
    get` raises `ClerkErrors` with an inherited `.status_code` on 4xx SDK-
    recognized errors, `SDKError` -- no reliable status code -- on other
    failures). Every other status and every exception fails closed
    ("error"), per issue #136's design: no deletion, logged, retried next
    sweep."""
    ...
    try:
        ...
        logger.exception("reconcile_deleted_users: Clerk SDK error looking up a candidate row")
        return "error"
    except Exception:
        logger.exception("reconcile_deleted_users: unexpected error looking up a candidate row")
        return "error"
    return "present"
...
def run_reconciliation_sweep(
    ...
    *,
    ...
    Returns {"checked": N, "deleted": N, "confirmed_present": N,
    ...
    see jobcannon.host.tasks.reconcile_deleted_users)."""
