...
def create_app(config: dict | None = None) -> Flask:
    ...
    if "WEBHOOK_SECRET" not in app.config:
        ...
        if not app.config.get("TESTING") and not secret:
            ...
            # fail-fast): an unset secret must never surface as per-request
            # 500s / invalid-signature noise instead of a clear boot failure.
            raise RuntimeError(
                "CLERK_WEBHOOK_SIGNING_SECRET is required (Svix webhook signing secret)"
            )
    ...
    @app.before_request
    def clerk_auth():
        if request.path in PUBLIC_PATHS or request.path.startswith(WEBHOOK_PREFIX):
            g.clerk_user = None
            return None
