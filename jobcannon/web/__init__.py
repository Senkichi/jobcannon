...
discoverable entry point anywhere on the public surface); that same
context processor also derives signup_cta_url — clerk_sign_up_url,
falling back to clerk_sign_in_url, but ONLY when the current visitor is
anonymous, else None — the single source every anonymous-visitor CTA
(the /preview page-level line, its per-row _posting_row.html CTA, and
the /demo CTA) gates on, so no template computes its own anonymous
fallback and no future public route can leak the CTA to a signed-in
visitor by forgetting to pass a flag (issue #174)."""
