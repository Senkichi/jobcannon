...
# ---------------------------------------------------------------------------
# Tests: format_local_iso (#1981; promoted from healthcheck.py's private
# _format_local once a second caller -- supervisor.py's doctor output --
# needed the identical naive-UTC-assumed -> local-aware conversion)
# ---------------------------------------------------------------------------


class TestFormatLocalIso:
    def test_none_returns_none(self):
        assert format_local_iso(None) is None

    def test_empty_string_returns_none(self):
        assert format_local_iso("") is None

    def test_unparseable_string_returns_none(self):
        assert format_local_iso("not-a-date") is None

    def test_naive_value_treated_as_utc_and_converted_to_local(self):
        result = format_local_iso("2026-08-28T12:00:00")
        assert result is not None
        parsed = datetime.fromisoformat(result)
        assert parsed.tzinfo is not None
        # Round-tripping through astimezone(UTC) must recover the original
        # naive-UTC wall-clock value exactly.
        from datetime import UTC

        assert parsed.astimezone(UTC).replace(tzinfo=None).isoformat() == "2026-08-28T12:00:00"

    def test_tz_aware_value_converts_directly(self):
        result = format_local_iso("2026-08-28T12:00:00+02:00")
        assert result is not None
        parsed = datetime.fromisoformat(result)
        assert parsed.tzinfo is not None
        from datetime import UTC

        assert parsed.astimezone(UTC).replace(tzinfo=None).isoformat() == "2026-08-28T10:00:00"
