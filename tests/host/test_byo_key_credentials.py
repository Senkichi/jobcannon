...
byo_key_credentials (L-0036 PR-1), plus the RLS policy m0019 adds.
...
def test_rls_blocks_cross_tenant_read_via_bare_select(db_conn):
    """m0019's tenant_isolation policy: a raw SELECT scoped to a DIFFERENT
...
def test_rls_blocks_read_with_no_session_var_set(db_conn):
    ...
    `user_id = NULL` is never true -- default-deny even after m0019's
    ...
    # upsert_credential's own _set_tenant call is LOCAL to its own internal
    # statement, not the ambient test transaction as a whole -- but to keep
    # this test's intent unambiguous (no caller has asked for any tenant),
    # explicitly reset rather than relying on that scoping detail.
