...
carry the shared marker class in its `class` attribute:
  ...
  - `h-11` AND `w-11` (both) for `type="checkbox"`/`type="radio"` inputs.
    ...
    <label>'s own size -- verified empirically that a native checkbox
    DOES honor explicit `h-11 w-11` (renders 44x44), so the control's own
    rect clears the floor rather than relying solely on the label acting
    as a de facto larger target.
...
def test_scan_found_a_plausible_number_of_interactive_elements():
    ...
    below the current true count (60+ across the templates directory) but
