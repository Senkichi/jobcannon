...
surface:
  ...
  * the serialization guards (issue #328): the manifest must be a pure
    function of the tree (no volatile `generated_at`) with one entry per
    line, so two port PRs adding disjoint entries merge cleanly under
    git's default three-way merge — pinned end-to-end against real
    `git merge-file`.
"""
