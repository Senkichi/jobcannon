"""Tests for ATS board-level scan concurrency (issue #1030).

Tests:
- Default (scan_concurrency=1) preserves the exact serial behavior, including
  the 0.5s inter-company sleep.
- scan_concurrency > 1 dispatches company scans across a bounded thread pool
  (recorded-concurrency-with-overlap: max simultaneous workers == the bound,
  not 1 and not more).
- get_scan_concurrency clamps out-of-range config values to [1, 6].
- Worker threads never touch the orchestrator's own sqlite3 connection (each
  opens its own via standalone_connection) — thread-identity capture on a
  spied orchestrator conn.
"""
...
def test_default_scan_concurrency_preserves_serial_behavior(migrated_db_path):
    """scan_concurrency=1 (the default) takes the serial path with the 0.5s sleep."""
    _insert_test_companies(migrated_db_path, count=2)
...
def test_concurrent_path_uses_multiple_threads(migrated_db_path):
    """scan_concurrency > 1 dispatches company scans across worker threads."""
    _insert_test_companies(migrated_db_path, count=4)

    summary = _base_summary()
    all_new_job_keys: list[str] = []
    thread_ids: list[int] = []

    original_standalone = standalone_connection

    def tracking_standalone(*args, **kwargs):
        thread_ids.append(threading.get_ident())
        return original_standalone(*args, **kwargs)

    with patch(
        "jobcannon.engine.ats_scanner._run.standalone_connection", side_effect=tracking_standalone
    ):
    ...
    assert len(set(thread_ids)) > 1, (
        ...
        f"from {len(thread_ids)} standalone_connection() calls"
    )
    assert summary["companies_scanned"] == 4


def test_scan_concurrency_bound_respected_with_overlap(migrated_db_path):
    ...
    """
    _insert_test_companies(migrated_db_path, count=8)
...
def test_concurrent_workers_never_touch_orchestrator_connection(migrated_db_path):
    ...
    (db_helpers.standalone_connection does not pass check_same_thread=False,
    so a real cross-thread conn share would raise sqlite3.ProgrammingError;
    this test would surface that as a captured company error even if the
    guard below didn't fire first).
    """
    ...
    with standalone_connection(migrated_db_path) as real_conn:
...
# (opened via standalone_connection) for upsert_job, while the worker path
...
def test_serial_and_concurrent_paths_are_equivalent(migrated_db_factory):
    ...
    """
    db_serial = migrated_db_factory()
    db_concurrent = migrated_db_factory()
    ...
    for field in ("companies_scanned", "jobs_discovered", "jobs_new"):
        assert summary_serial[field] == summary_concurrent[field], (
            f"{field}: serial={summary_serial[field]} concurrent={summary_concurrent[field]}"
        )
