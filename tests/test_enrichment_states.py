...
def test_expired_is_terminal_and_low_signal():
    """T2.9 / D21: 'expired' is the terminal end-state after the agentic_exhausted
    retry budget is spent with no jd_full recovered — the cascade gave up exactly
    like agentic_exhausted, so it belongs in both predicate sets."""
...
def test_low_signal_terminal_matches_legacy_frozenset():
    ...
    plus 'expired' (T2.9 / D21 — a gave-up terminal exactly like agentic_exhausted)."""
